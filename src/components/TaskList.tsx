...
interface TaskListProps {
  tasks: Task[];
  selectedDate: string; // new prop to track current date (format: 'yyyy-MM-dd')
...
const TaskList: React.FC<TaskListProps> = ({
  ...
  // Helper: Get status for task on selected date (for event tasks, use perDayStatus)
  ...
  // Handle checkbox change: update perDayStatus for event tasks, else status
  const handleCheckboxChange = (taskId: string, checked: boolean) => {
    const task = tasks.find((t) => t.id === taskId);
    if (!task) return;

    if (task.isEvent) {
      // Update perDayStatus only for selectedDate
      const newPerDayStatus = { ...task.perDayStatus, [selectedDate]: checked ? 'completed' : 'pending' };
      onUpdateTask(taskId, { perDayStatus: newPerDayStatus });
    } else {
      // Normal task - update status as usual
      onUpdateTask(taskId, { status: checked ? 'completed' : 'pending' });
    }
  };
  ...
  const activeTasks = tasks.filter((task) => getStatusForTaskDate(task, selectedDate) !== 'completed');
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1 sm:mb-2">
                  ...
                  {!isCompleted && (
                    <Badge
                      variant="outline"
                      className={`text-xs sm:text-sm ${getStatusColor(statusForDate)}`}
                    >
            ...
            <div className="flex items-center justify-end sm:justify-normal gap-1 sm:gap-2 mt-2 sm:mt-0">
              {!isCompleted && (
                <>
                  {/* Note: For event tasks, changing the global status is optional. You might want to disable this Select or handle it differently */}
                  <Select
                    value={task.status}
